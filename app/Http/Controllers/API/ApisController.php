...
use AleeDhillon\MetaFive\Entities\User as meta44;
...
class ApisController extends Controller
{
    ...
    public function test(){
        ...
        $user->setGroup("demo\\demoforex");  // Corrected group format
        $user->setLeverage(50);  // Ensure leverage is an integer and valid
        $user->setPhone("20155498729");  // Adjust phone format if needed
        $user->setAddress("Lahore");
        $user->setCity("Lahore");
        $user->setState("Punjab");
        $user->setCountry("Pakistan");
        $user->setZipCode(1470);
        $user->setMainPassword("Secr3tP@ssw0rd!");  // Ensure password complexity
        $user->setInvestorPassword("Secr3tP@ssw0rd!");
        $user->setPhonePassword("Secr3tP@ssw0rd!");
        ...
        if ($login === null) {
            throw new Exception('Login key not found in the response.');
        }
        ...
        // $c=new Client();
        // $result = $c->createUser($user);
        // return $result;

    //     $trade = new Trade();

    // $trade->setLogin(287001);
